...
const branch = params.branch as string;       // cse
const semester = params.semester as string;   // sem-4
const subjectFromUrl = params?.subject as string | undefined;
const paperFromUrl = params?.paper as string | undefined;


const handleSubjectChange = (subject: any) => {
    if (!subject) return;
  
    const paperYear = activePaper?.year || "";
  
    router.push(
      `/pyq/${branch}/${semester}/${subject.code}/${paperYear}`,
  ...
  const handlePaperChange = (paper: any) => {
    if (!paper || !activeSubject) return;
  
    router.push(
      `/pyq/${branch}/${semester}/${activeSubject.code}/${paper.year}`,
      { scroll: false }
    );
  };
...
/*------------------  AUTO URL SYNC  -----------*/
  useEffect(() => {
    ...
    const correctUrl = `/pyq/${branch}/${semester}/${activeSubject.code}/${activePaper.year}`;
  
    if (window.location.pathname !== correctUrl) {
      router.replace(correctUrl);
    }
  }, [activeSubject, activePaper, branch, semester]);
  ...
  const semCode = semester.replace("sem-", "");

  apiGet(`/user/subjects?semester_code=${semCode}&branch_code=${branch}`)
    .then(json => {
    ...
    //   if (theoryOnly?.length) setActiveSubject(theoryOnly[0]);
      let selected = theoryOnly.find((s: any) => s.code === subjectFromUrl);
...
}, [branch, semester, subjectFromUrl]);
...
useEffect(() => {
  ...
  .then(json => {
    ...
    let selected = sorted.find(
        (p: any) => String(p.year) === paperFromUrl
      );
    ...
    <div className="bg-white rounded-xl p-6 shadow-xl text-center">
      ...
      >
        ...
        //   onPaper={setActivePaper}
          onSubject={(s) => handleSubjectChange(s)}
          onPaper={(p) => handlePaperChange(p)}
